...
function formatEthFromWei(weiValue: number | string): string {
  const ethValue = formatEther(BigInt(weiValue.toString()));
...
const MarketCard: React.FC<MarketCardPropsWithWallet> = ({ market, votes, userAddress, address, signer, connect, connecting }) => {
  ...
  // On-chain resolved tallies (store as numbers like the original working version)
  ...
  useEffect(() => {
    const fetchOnChainVotes = async () => {
      ...
      try {
        ...
        const yes = Number((betInfo.totalYesVotes as any)?.toString?.() ?? betInfo.totalYesVotes ?? 0);
        const no = Number((betInfo.totalNoVotes as any)?.toString?.() ?? betInfo.totalNoVotes ?? 0);
        setResolvedYes(yes);
        setResolvedNo(no);
      } catch (err) {
      }
  ...
  useEffect(() => {
    const fetchResolvedTallies = async () => {
      ...
      try {
        ...
        const yes = Number((betInfo.yesVotes as any)?.toString?.() ?? betInfo.yesVotes ?? 0);
        const no = Number((betInfo.noVotes as any)?.toString?.() ?? betInfo.noVotes ?? 0);
        setResolvedYes(yes);
        setResolvedNo(no);
      } catch (err) {
        setResolvedYes(null);
  ...
  //       setClaimAmount(formatEther(BigInt(amount.toString())));
  //     });
  //   }
  ...
  const handleClaim = async () => {
    setClaiming(true);
    try {
      let effectiveSigner = signer as any;
      if (!effectiveSigner) {
      ...
      const targetAddr = (contract && (contract.target ?? contract.address)) as unknown as string;
      const contractWithABI = new ethers.Contract(targetAddr, BeliefMarketABI.abi, effectiveSigner);
      await contractWithABI.claimPrize(market.betId);
  ...
  const handleRefund = async () => {
    ...
    try {
      ...
      const targetAddr = (contract && (contract.target ?? contract.address)) as unknown as string;
      const contractWithABI = new ethers.Contract(targetAddr, BeliefMarketABI.abi, effectiveSigner);
      await contractWithABI.claimRefund(market.betId);
      // Refresh hasClaimed status
      if (contract && market.betId && address) {
        const claimed = await contract.hasUserClaimed(market.betId, address);
        setHasClaimed(claimed);
  ...
  if (betInfo) {
    if (
      ...
      const userWeight = BigInt(betInfo.voteStake ? betInfo.voteStake : 0);
      const totalWinningWeight = yesWon ? BigInt(betInfo.yesVotes) : BigInt(betInfo.noVotes);
      const prizePool = BigInt(betInfo.prizePool ? betInfo.prizePool : 0);
      eligibleAmount = (totalWinningWeight > 0n)
        ? formatEther((prizePool * userWeight) / totalWinningWeight)
        : "0.0";
    } else if (isResolved && isTie && userVote) {
      // Tie refund
      const voteStake = BigInt(betInfo.voteStake ? betInfo.voteStake : 0);
      eligibleAmount = formatEther(voteStake);
    }
  }
        ...
        {isExpired && !isResolved && address === market.creator ? (
          <button
            ...
            onClick={async (e) => {
              ...
              setVotePending(true);
              try {
                // Ensure we have an Ethers v6 Signer for writes
                let effectiveSigner = signer as any;
                if (!effectiveSigner) {
                  if (typeof window !== 'undefined' && (window as any).ethereum) {
                    const provider = new ethers.BrowserProvider((window as any).ethereum);
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (selectedOption && userAddress && !userVote) {
                    try {
                      try {
                        const resolvedContractAddress = ((contract && (contract.target ?? contract.address)) as unknown as string);
                        const stakeWei = parseEther(market.minBet.toString()).toString();
                        // Debug logs to inspect what we send into the FHE relayer flow
                        console.groupCollapsed('[Vote] Preparing confidential vote payload');
                        console.log('betId:', market.betId);
                        console.log('marketId:', market.id);
                        console.log('voteType (0=no,1=yes):', selectedOption === 'yes' ? 1 : 0);
                        console.log('stake (wei):', stakeWei);
                        console.log('contractAddress:', resolvedContractAddress);
                      ...
                      await castConfidentialVote({
                        contractAddress: ((contract && (contract.target ?? contract.address)) as unknown as string),
                        abi: BeliefMarketABI.abi,
                        betId: market.betId,
                        marketId: market.id, // Pass the Firestore document ID
                        voteType: selectedOption === 'yes' ? 1 : 0,
                        voteStake: parseEther(market.minBet.toString()).toString(),
                        signer,
                      ...
                      console.error('[Vote] castConfidentialVote failed:', err);
                      setError(getFriendlyErrorMessage(err));
                    }
                    setVotePending(false);
                  }
